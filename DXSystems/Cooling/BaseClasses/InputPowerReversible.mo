...
block InputPowerReversible "Electrical power consumed by the unit"
  ...
  Modelica.Blocks.Interfaces.IntegerInput uMod
   "Control input signal, cooling mode=-1, off=0, heating mode=+1"
    annotation (Placement(transformation(extent={{10,-10},{-10,10}},
        ...
        rotation=90,
        origin={0,-110})));
  Modelica.Blocks.Sources.RealExpression powGai(y=u_power)
    annotation (Placement(transformation(extent={{-10,22},{10,42}})));
  Modelica.Blocks.Math.Product powPro
...
equation
  ...
  connect(powGai.y, powPro.u2) annotation (Line(points={{11,32},{20,32},{20,54},
          {38,54}}, color={0,0,127}));
  connect(proP.y, powPro.u1) annotation (Line(points={{11,60},{20,60},{20,66},{
          38,66}}, color={0,0,127}));
  connect(powPro.y, P)
