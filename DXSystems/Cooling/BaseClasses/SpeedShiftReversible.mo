...
block SpeedShiftReversible "Interpolates values between speeds"
  ...
  parameter Real funSpeCoo[3] "Coefficients of function of speed ratio for cooling";
  parameter Real funSpeHea[3]
                             "Coefficients of function of speed ratio for heating";

  Modelica.Blocks.Interfaces.RealOutput facSpeRat "Speed ratio correction factor";
...
equation
  ...
  if uMod == -1 then
    facSpeRat = (funSpeCoo[1] + funSpeCoo[2]*speRat+ funSpeCoo[3]*speRat^2);
  elseif uMod == 1 then
    facSpeRat = (funSpeHea[1] + funSpeHea[2]*speRat + funSpeHea[3]*speRat^2);
  else
    facSpeRat = 0;
  end if;
...
<p>This block uses the <a href=\"modelica://Buildings.Fluid.DXSystems.Cooling.BaseClasses.Functions.speedShift\">Buildings.Fluid.DXSystems.Cooling.BaseClasses.Functions.speedShift</a> function to interpolate the input array. Depending on input speed ratio and speed set array the input array <i>u</i> is interpolated. </p>
<p>Update:</p>
<p>1. 24/06/14 linear interpolation</p>
</html>",
revisions="<html>
<ul>
<li>
