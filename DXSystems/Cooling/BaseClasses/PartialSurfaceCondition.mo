...
partial block PartialSurfaceCondition
  ...
  extends DXSystems.Cooling.BaseClasses.EssentialParameters;
  ...
  output Modelica.Units.SI.AngularVelocity spe(displayUnit="1/min")
    "Rotational speed";
  DXSystems.Cooling.BaseClasses.UACp uacp[nSta](per=datCoi.sta.nomVal,
...
equation
  if variableSpeedCoil or stage <> 0 then
    ...
    UAcp = if variableSpeedCoil then
      Buildings.Utilities.Math.Functions.smoothMax(
      x1=DXSystems.Cooling.BaseClasses.Functions.speedShift(
        spe=spe,
        speSet={datCoi.sta[iSpe].spe for iSpe in 1:nSta},
        u={uacp[iSpe].UAcp for iSpe in 1:nSta}),
      x2=uacp[nSta].UAcp/1E3,
      deltaX=uacp[nSta].UAcp/1E4) else uacp[stage].UAcp;
