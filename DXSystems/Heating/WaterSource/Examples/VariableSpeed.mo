within DXSystems.Heating.WaterSource.Examples;
model VariableSpeed "Test model for variable speed DX coil"
  ...
  parameter Modelica.Units.SI.PressureDifference dpCon_nominal=40000
    "Pressure drop at mSou_flow_nominal";
  ...
  DXSystems.Heating.WaterSource.VariableSpeed varSpeDX(
    redeclare package MediumEva = MediumWater,
  ...
  parameter DXSystems.Heating.WaterSource.Data.Generic.DXCoil datCoi(nSta=1,
...
equation
  connect(TEvaIn.y, souAir.T_in) annotation (Line(
      ...
      color={0,0,127},
      smooth=Smooth.None));
  connect(varSpeDX.portEva_a, souWat.ports[1]) annotation (Line(points={{10,-6},
          {28,-6},{28,-46},{32,-46}}, color={0,127,255}));
  connect(varSpeDX.portEva_b, sinWat.ports[1]) annotation (Line(points={{-2,-6},
  ...
  connect(speRat.y, varSpeDX.speRat) annotation (Line(points={{-69,12},{-7.2,12},
