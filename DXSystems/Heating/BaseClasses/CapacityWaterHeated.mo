...
block CapacityWaterHeated
  "Calculates cooling capacity at given temperature and flow fraction for water source DX coils"
  extends DXSystems.Cooling.BaseClasses.PartialCapacity(final use_mCon_flow=
        true, redeclare
      DXSystems.Heating.WaterSource.Data.Generic.BaseClasses.Stage sta[nSta]);
...
initial algorithm
  // Verify correctness of performance curves, and write warning if error is bigger than 10%
   for iSta in 1:nSta loop
    ...
    DXSystems.Cooling.BaseClasses.Functions.warnIfPerformanceOutOfBounds(
      Buildings.Fluid.Utilities.extendedPolynomial(
        x=1,
        c=sta[iSta].perCur.capFunFFSou,
        xMin=sta[iSta].perCur.ffSouMin,
        xMax=sta[iSta].perCur.ffSouMax),
      msg=
        "Capacity as a function of normalized water mass flow rate at the condenser",
      curveName="sta[" + String(iSta) + "].perCur.capFunFFSou");
    ...
    DXSystems.Cooling.BaseClasses.Functions.warnIfPerformanceOutOfBounds(
      Buildings.Fluid.Utilities.extendedPolynomial(
        ...
        xMax=sta[iSta].perCur.ffSouMax),
      msg=
        "EIR as a function of normalized water mass flow rate at the condenser",
      curveName="sta[" + String(iSta) + "].perCur.EIRFunFFSou");
   end for;
...
if stage > 0 then
    ...
    ffCon[iSta]=Buildings.Utilities.Math.Functions.smoothMax(
      ...
      x2=m_flow_small,
      deltaX=m_flow_small/10)/sta[iSta].nomVal.mSou_flow_nominal;
  //-------------------------Cooling capacity modifiers----------------------------//
    ...
    corFacCon[iSta] =Buildings.Utilities.Math.Functions.smoothHeaviside(
       x=ffCon[iSta] - sta[iSta].perCur.ffSouMin/4,
       delta=max(Modelica.Constants.eps, sta[iSta].perCur.ffSouMin/8));
...
<p>
  ...
  See <a href=\"modelica://Buildings.Fluid.DXSystems.Cooling.BaseClasses.PartialCapacity\">
  Buildings.Fluid.DXSystems.Cooling.BaseClasses.PartialCapacity</a> for more information.
</p>
</html>",
revisions="<html>
<ul>
