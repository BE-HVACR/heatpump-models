within DXSystems.Heating.AirSource.Examples;
model SingleSpeed "Test model for single speed DX heating coil"
  ...
  parameter DXSystems.Heating.AirSource.Data.Generic.DXCoil datCoi(
    nSta=1,
    minSpeRat=0.2,
    sta={DXSystems.Heating.AirSource.Data.Generic.BaseClasses.Stage(
        spe=1800/60,
        nomVal=
          DXSystems.Heating.AirSource.Data.Generic.BaseClasses.NominalValues(
          Q_flow_nominal=16381.47714,
          COP_nominal=3.90494,
          m_flow_nominal=2,
          TEvaIn_nominal=273.15 - 5,
          TConIn_nominal=273.15 + 21),
        perCur=DXSystems.Heating.AirSource.Examples.PerformanceCurves.Curve_I())},
    final defOpe=DXSystems.Heating.BaseClasses.Types.DefrostOperation.resistive,
...
equation
  ...
  connect(sinSpeDX.port_a, sou.ports[1]) annotation (Line(
      ...
      color={0,127,255},
      smooth=Smooth.None));
  connect(TEvaIn.y, sinSpeDX.TOut) annotation (Line(points={{-59,-4},{19,-4}},
                         color={0,0,127}));
  connect(sinSpeDX.port_b, sin.ports[1]) annotation (Line(
      points={{40,0},{44,0},{44,-20},{48,-20}},
      color={0,127,255},
      smooth=Smooth.None));
  connect(sinSpeDX.phi, phi.y) annotation (Line(points={{19,-8},{-50,-8},{-50,
          40},{-59,40}},
                     color={0,0,127}));
